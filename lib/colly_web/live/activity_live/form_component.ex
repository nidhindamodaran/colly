defmodule CollyWeb.ActivityLive.FormComponent do
  ...
  def update(%{activity: activity} = assigns, socket) do
    changeset = Collab.change_activity(activity)
  ...
  def handle_event("validate", %{"activity" => activity_params}, socket) do
    changeset =
      ...
      |> Collab.change_activity(activity_params)
  ...
  defp save_activity(socket, :edit, activity_params) do
    case Collab.update_activity(socket.assigns.activity, activity_params) do
      {:ok, _activity} ->
        {:noreply,
         socket
  ...
  defp save_activity(socket, :new, activity_params) do
    case Collab.create_activity(activity_params) do
      {:ok, _activity} ->
        {:noreply,
         socket
